...
############################################# outer loop ##############################################
 ...
 SD_volume <- 0.3* 0.453592 * 152694720  # 30% CV
 volume.sample <-rtruncnorm (n=M, a = qnorm(0.05, mean = 0.453592 * 152694720, sd = SD_volume  ), 
                    b = qnorm(0.95, mean = 0.453592 * 152694720, sd = SD_volume ), 
                    mean = 0.453592 * 152694720, sd = SD_volume )             # kg 
 quantile(volume.sample, c(0,0.05,.5, 0.75, 0.9, 0.95,1)) 
 
 ##### percentage in plastic (Sc, shared among members of the entire cohort)
 SD_perc <- 0.3*0.5
 perc.sample <-rtruncnorm (n=M, a = 0.01, 
                   b = 0.8, 
                   mean = 0.3, sd =  SD_perc) 
 quantile(perc.sample, c(0, 0.05,.5, 0.75, 0.9, 0.95,1))      #(Sc, shared among members of the entire cohort)
 ...
 SD_kp <- (0.23 - 0.06) / qnorm(0.95)     #(Sc, shared among members of the entire cohort)
 Kp.sample <-rtruncnorm (n=M, a = 0.005, 
                  b = qnorm(0.95, mean = 0.06, sd = SD_kp), 
                  mean = 0.06, sd = SD_kp)              ## m3/ug
 quantile(Kp.sample, c(0.05,.5, 0.75, 0.9, 0.95)) 
 
 ## vapor from product #(Sc, shared among members of the entire cohort)
 #SD_y0  <- 1 * 2.5  # 30% CV
 #y0.sample  <- rtruncnorm (n=M, a = 0.05, 
 #                   b = 2.5, 
 #                   mean =  2.5 , sd = SD_y0)             ## ug/m3 
 
 ##kpf of DEHP  #(ScUc, highly uncertain)
 kpf_mean.sample <- runif(n=M, min = 5126, max = 22458) #40-50% etoh content 
 ...
 Sc.sample <- cbind(volume.sample, perc.sample,life_mean.sample, life_CV.sample,  occurance.sample ,
 ...
 for (i in 1:M){
   ...
   ## concentration in food contacting material Cp (%) log-normal # # (Sc, shared among members of the entire cohort)
   #dCp <- c(0.0005, 0.05)
   #PercdCp <- c(0.5, 0.9)
   #c <- get.lnorm.par(p= PercdCp,q=dCp)
   Cp_ori <- rlnormTrunc(n=N, meanlog = -7.601138, sdlog =  3.593151, max = 0.5) ## g/g
   Cp <- Cp_ori * 1E6   ## ug/g  # unshared (Us)
   #quantile(Cp_ori*100, c(0,.05, .1,.25,.5, 0.75, 0.9, 0.95, 1))
...
###########################################################
 ...
 df.inhal$DEHP <- c("Inhalation") 
 #names(df.inhal)=c("Value","DEHP")

 ## Oral
 df.oral<-as.data.frame(df.OE)
 df.oral = subset(df.oral, select = -c(V1) )
 df.oral$DEHP <- c("Oral") 
 #names(df.oral)=c("Value","DEHP")

 ## Dermal
 df.dermal<-as.data.frame(df.SE)
 df.dermal = subset(df.dermal, select = -c(V1) )
 df.dermal$DEHP <- c("Dermal") 
 #names(df.dermal)=c("Value","DEHP")
 ...
 x.median <- c(2.3, 0.76, 0.75, 1.43, 2.2, 2.4, 4.2)
 x.95 <- c(20.2, 3.3, 3.2, 6.77, 5.7, 18.8, 21)
 survey.median <- quantile(x.median, c(.95))
 survey.95 <- quantile(x.95, c(.95))
 
 SD_survey <- (survey.95 - survey.median) / qnorm(0.95)
 survey <- rlnormTrunc(n=N, meanlog = survey.median, sdlog =  SD_survey, min = 1e-10, max = 100)
 hist(survey, breaks = 50)  
   # rtruncnorm  (n=N, a = 1e-05, 
      #                 b = qnorm(0.95, mean =survey.median , sd = SD_survey),
         #              mean = survey.median, sd = SD_survey)  
 
 lst.ks <- lapply(1:ncol(df.all), function(i)
    #ks.test(df.all[, i], survey, alternative = "l" )$p.value)
  JSD(rbind(df.all[, i], survey), unit = 'log2'))
 lst.ks <- as.data.frame(lst.ks)

 #hist(df.all[, 2],breaks = 50)
 #descdist(df.all[, 2], discrete = FALSE)
 ...
 #df.f <- df.50[, 1:50]
 df.f2 <- cbind.data.frame(df.f, df.2R$DEHP)
 names(df.f2)[length(names(df.f2))]<-"DEHP" 
 
 # extrace subset data frame for inhal, dermal,  oral, all sepearately
 df.inhal <- df.f2[df.f2$DEHP == 'Inhalation', ]
 df.dermal <- df.f2[df.f2$DEHP == 'Dermal', ]
 df.oral <- df.f2[df.f2$DEHP == 'Oral', ]
 df.all <- df.f2[df.f2$DEHP == 'all', ]
 
 # mix all data within each category
 df.inhal.A = as.data.frame(subset(df.inhal, select = -c(DEHP) ))
 df.inhal <- data.frame(x=unlist(df.inhal.A))
 df.inhal$DEHP <- c("Inhalation") 
 names(df.inhal)=c("Value","DEHP")
 
 df.dermal.A = as.data.frame(subset(df.dermal, select = -c(DEHP) ))
 df.dermal <- data.frame(x=unlist(df.dermal.A))
 df.dermal$DEHP <- c("Dermal") 
 names(df.dermal)=c("Value","DEHP")
 
 
 df.oral.A = as.data.frame(subset(df.oral, select = -c(DEHP) ))
 df.oral <- data.frame(x=unlist(df.oral.A))
 df.oral$DEHP <- c("Oral") 
 names(df.oral)=c("Value","DEHP")
 
 
 df.all.A = as.data.frame(subset(df.all, select = -c(DEHP) ))
 df.all <- data.frame(x=unlist(df.all.A))
 df.all$DEHP <- c("all") 
 names(df.all)=c("Value","DEHP")
...
p1 = 
  ggplot (df, aes(x = as.numeric(log.value), y = as.factor(ggjoy),fill = DEHP)) + # fill, fill one color for each species
...
ggsave("Exposure_updated_4a.tiff",scale = 1,
       plot = p1,
       path = "C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK",
...
saveRDS(IE,file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/IE.rds')
saveRDS(OE,file='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/OE.rds')
saveRDS(SE,file='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/SE.rds')
saveRDS(BW,file='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/BW.rds')
saveRDS(df,file='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/df.rds')
saveRDS(df.all,file='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/df.all.rds')

IE = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/IE.rds')
OE = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/OE.rds')
SE = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/SE.rds')
df = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/df.rds')
df.all = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/df.all.rds')
OE = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/OE.rds')
IE = readRDS(file ='C:/Users/Punkostrich/Dropbox/NM/Class/DEHP/PK/DEHP/result/IE.rds')

quantile(df.all, c(0.01,.05,0.25, 0.5, 0.75,.95,0.99,1))
quantile(OE, c(0.01,.05,0.25, 0.5, 0.75,.95,0.99,1))
quantile(IE, c(0.01,.05,0.25, 0.5, 0.75,.95,0.99,1))
